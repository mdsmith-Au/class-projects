...
int mainLoop() {
    ...
    for (;;) {
        ...
        // Receive data from client_socket, and send it to the dispatcher
...
/*
 * Note: I modified heartbeat to take the IP of the namenode, since
 * it was passed as an argument but never passed
 */
static void *heartbeat(char* namenode_ip) {
    ...
    for (;;) {
        ...
        // Connection open, send info back to namenode
...
int start(int argc, char **argv) {
    ...
    create_thread((void *)heartbeat, namenode_ip);
...
int read_block(int client_socket, const dfs_cli_dn_req_t *request) {
    ...
    dfs_cli_dn_req_t response;
    memcpy(&response, request, sizeof(response));
    
    ext_read_block(request->block.owner_name, request->block.block_id, (void *) buffer);

    // Data now in buffer, copy it back to the response
    memcpy(response.block.content, buffer, sizeof (buffer));
...
void requests_dispatcher(int client_socket, dfs_cli_dn_req_t request) {
    printf("Datanode request type %i received\n", request.op_type);
